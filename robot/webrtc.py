...
class RoboRTCPeerConnection:
    ...
    async def create_track(self, kind: str, name: str):
        """
        Create a track.
        """
        track = None
    ...
    async def _on_track(self, track: MediaStreamTrack):
        ...
        @track.on("ended")
        def on_ended():
            self.logger.debug(f"Track {track.kind} ended")
...
class SignalingClient:
    ...
    def __init__(
            self,
            client: RTCClient,
            ice_servers: Optional[RTCIceServer] = None,
            logger=None
    ):
        """
    ...
    def register_socket_envent(self):
        if self._sio is not None:
            asyncio.ensure_future(self._sio.disconnect())
    ...
    def disconnect_events(self):
        # self._sio.off(SocketEvents.CONNECT.value)
        # self._sio.off(SocketEvents.DISCONNECT.value)
        # self._sio.off(SocketEvents.CLOSE.value)
        # self._sio.off(SocketEvents.ROOM_CLIENTS.value)
        # self._sio.off(SocketEvents.MAKE_PEER_CALL.value)
        # self._sio.off(SocketEvents.PEER_CALL_RECEIVED.value)
        # self._sio.off(SocketEvents.PEER_CALL_ANSWER_RECEIVED.value)
        # self._sio.off(SocketEvents.PEER_CALL_ICE_CANDIDATE_RECEIVED.value)
        pass
    ...
    async def _on_room_clients(self, clients: List):
        ...
        for client in clients:
            ...
            if room != self.client.room or roomId != self.client.roomId:
                continue
            self.logger.debug(f"new client: {client} - {self.kind}")
    ...
    async def _make_peer_call(self, _id: str):
        ...
        if getattr(client.pc, "is_connected", False):
            return
        rtc_connection = await self.create_connection(client, is_caller=True)
    ...
    async def _on_peer_call_received(self, data: Dict):
        ...
        if getattr(client.pc, "is_connected", False):
            return
        rtc_connection = await self.create_connection(client, is_caller=False)
        rtc_connection.on("icecandidate", self._on_ice_candidate)
        client.pc = rtc_connection
    ...
    async def on_datachannel(self):
        ...
        while 1:
            ...
            for s in status_dict:
                my_data = json.dumps(s)
                print(">>>>>>>", my_data, self._data_channel.readyState)
...
class ControlRTCRobot(ClientBase):  # noqa
    def __init__(self, robot, name: str = "control", loop=None, **kwargs, ):
        """
    ...
    def add_worker(self,
                   ...
                   ice_server: Optional[RTCIceServer] = None,
        ...
        :param ice_server: The ice server.
