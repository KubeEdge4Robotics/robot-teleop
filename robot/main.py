...
class RoboRTCClient(ControlRTCRobot):  # noqa
    def __init__(self,
                 robot,
                 name: str = "control",
                 auth_token: str = "",
        ...
        super().__init__(robot, name, loop, **kwargs)
    ...
    def run(self):
        ...
        parsed_url = urllib.parse.urlparse(self.uri)  # noqa
        _path = parsed_url.path.strip('/')
        if not _path.endswith(engineio_path):
            _path = _path + '/' + engineio_path
        query = []
        if parsed_url.query:
            query.append(parsed_url.query)
        if self.auth_token:
            query.append(f"x-auth-token={self.auth_token}")
        ...
        for n, w in self._workers.items():
            main_task = asyncio.ensure_future(
                w.async_run(url, socketio_path=_path),
                loop=self.loop
            )
...
class DogDanceSkill(SkillBase):  # noqa
    def __init__(self, robot):
        super(DogDanceSkill, self).__init__(name="DogDance", robot=robot)
...
class RoboClient:
    ...
    def run(self):
        self.robot.skill_register("dance", DogDanceSkill)
    ...
    def command_callback(self, msg: Dict):
        ...
        if _type == "waypoint":
            ...
            return
        if (self.navigation is not None) and (_type == "start"):
